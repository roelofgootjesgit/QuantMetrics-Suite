...
class TestBBOnlySignals:
    ...
    def test_collect_entries_filters_opposite_direction_clusters(self, monkeypatch):
        ...
        bands = pd.DataFrame(
            {
                "lower": np.full(len(df), 0.5),
                "mid": np.full(len(df), 1.0),
                "upper": np.full(len(df), 1.5),
            },
            index=df.index,
        )
        long_raw = pd.Series(
        ...
        monkeypatch.setattr("src.quantbuild.strategies.bb_only.compute_bb_bands", lambda *_: bands)
        monkeypatch.setattr(
            "src.quantbuild.strategies.bb_only.detect_bb_component_observations",
            lambda *_: (long_raw, short_raw),
        )
        monkeypatch.setattr(
            "src.quantbuild.strategies.bb_only.compute_atr",
...
class TestBBMidlineSimulator:
    ...
    def test_sl_checked_when_midline_missing(self):
        n = 12
        dates = pd.date_range("2024-01-01", periods=n, freq="15min", tz="UTC")
        close = np.full(n, 1.0)
        low = np.full(n, 0.999)
        low[6] = 0.97
        df = pd.DataFrame(
            {"open": close, "high": close + 0.001, "low": low, "close": close},
            index=dates,
        )
        mid = np.full(n, 1.0)
        mid[6] = np.nan
        ...
        res = simulate_bb_midline_trade(
            df, 5, "LONG", mid=mid, atr_arr=atr, sl_atr_mult=2.0, time_exit_bars=4
        )
