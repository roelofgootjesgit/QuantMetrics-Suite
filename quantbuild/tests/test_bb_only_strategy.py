...
class TestBBMidlineSimulator:
    ...
    def test_long_wick_touch_hits_midline_before_close(self):
        n = 6
        dates = pd.date_range("2024-01-01", periods=n, freq="15min", tz="UTC")
        ...
        atr = np.full(n, 0.01)
        res = simulate_bb_midline_trade(
            df, 0, "LONG", mid=mid, atr_arr=atr, sl_atr_mult=10.0, time_exit_bars=4
        )
        assert res["exit_reason"] == "midline"
        assert res["exit_bar_idx"] == 1

    def test_short_wick_touch_hits_midline_before_close(self):
        n = 6
        dates = pd.date_range("2024-01-01", periods=n, freq="15min", tz="UTC")
        ...
        atr = np.full(n, 0.01)
        res = simulate_bb_midline_trade(
            df, 0, "SHORT", mid=mid, atr_arr=atr, sl_atr_mult=10.0, time_exit_bars=4
        )
        assert res["exit_reason"] == "midline"
        assert res["exit_bar_idx"] == 1
