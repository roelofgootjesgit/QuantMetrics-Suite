...
def _merge_parquet_cache(
    base_path: Path,
    symbol: str,
    timeframe: str,
    fetched: pd.DataFrame,
) -> None:
    """Merge refreshed bars into the cache without discarding older history."""
    cached = load_parquet(base_path, symbol, timeframe)
    if cached.empty:
        save_parquet(base_path, symbol, timeframe, fetched)
        return

    merged = pd.concat([cached, fetched])
    merged.index = pd.to_datetime(merged.index, utc=True).tz_localize(None)
        ...
        try:
            ...
            if provider == "ctrader":
                ...
                if not data.empty and len(data) >= 100:
                    _merge_parquet_cache(base_path, symbol, timeframe, data)
