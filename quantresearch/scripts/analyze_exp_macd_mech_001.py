...
def _directional_permutation_test(
    ...
    *,
    ...
    for i, direction in zip(signal_indices, signal_directions):
        r = _forward_return_r(data, atr_series, int(i), direction, horizon)
        if r is None:
            continue
    ...
    eligible = np.array(
        [
            ...
            if np.isfinite(atr_arr[i]) and atr_arr[i] > 0
        ],
        dtype=int,
    )
    if len(eligible) == 0:
        return {
            "observed_hit_rate": float(np.mean(observed)),
            "baseline_mean_hit_rate": 0.0,
            "p_value": 1.0,
            "significant": False,
    ...
    replace = len(eligible) < n_signals
    for perm_i in range(n_permutations):
        random_idx = rng.choice(eligible, size=n_signals, replace=replace)
        random_returns = [
            _forward_return_r(data, atr_series, int(i), direction, horizon) or 0.0
            for i, direction in zip(random_idx, valid_directions)
        ]
        perm_rates[perm_i] = float(np.mean(random_returns))
