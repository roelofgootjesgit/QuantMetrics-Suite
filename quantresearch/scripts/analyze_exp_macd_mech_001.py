...
def _directional_permutation_test(
    ...
    *,
    ...
    """Random-timestamp test preserving each signal's LONG/SHORT direction."""
    close = data["close"].values.astype(float)
    atr = atr_series.values.astype(float)
    if len(close) == 0:
        raise ValueError("data must not be empty")

    max_start = len(close) - int(horizon)
    if max_start <= 0:
        return {
    ...
    risk = 2.0 * atr[:max_start]
    with np.errstate(divide="ignore", invalid="ignore"):
        long_r = (close[int(horizon):] - close[:max_start]) / risk
    valid = np.isfinite(long_r) & np.isfinite(risk) & (risk > 0)
    ...
    for sig in signals:
        i = int(sig["bar_index"])
        if i < 0 or i >= max_start or not valid[i]:
            continue
        direction = str(sig["direction"]).upper()
        sign = 1.0 if direction == "LONG" else -1.0
        observed.append(float(long_r[i] * sign))
        direction_signs.append(sign)

    if not observed or len(valid_indices) == 0:
        return {
            "observed_hit_rate": 0.0,
            "baseline_mean_hit_rate": 0.0,
            "p_value": 1.0,
    ...
    signs = np.asarray(direction_signs, dtype=float)
    observed_rate = float(np.mean(observed))
    rng = np.random.default_rng(seed)
    perm_rates = np.empty(n_permutations, dtype=float)
    replace = len(signs) > len(valid_indices)
    for i in range(n_permutations):
        random_idx = rng.choice(valid_indices, size=len(signs), replace=replace)
        perm_rates[i] = float(np.mean(long_r[random_idx] * signs))

    p_value = float(np.mean(perm_rates >= observed_rate))
    return {
        "observed_hit_rate": observed_rate,
        "baseline_mean_hit_rate": float(np.mean(perm_rates)),
        "p_value": p_value,
