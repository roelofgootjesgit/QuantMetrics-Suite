    ...
    *,
    ...
    for idx, direction in zip(signal_indices, signal_directions):
        ...
        if idx < 0 or idx >= n_bars or direction_u not in {"LONG", "SHORT"}:
            continue
        long_r = long_returns[idx]
        if not np.isfinite(long_r):
            continue
        observed_returns.append(float(long_r if direction_u == "LONG" else -long_r))
    ...
    long_count = sum(1 for d in valid_directions if d == "LONG")
