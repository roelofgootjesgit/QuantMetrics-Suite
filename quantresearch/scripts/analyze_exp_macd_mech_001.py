...
def _directional_t8_permutation_test(
    ...
    *,
    ...
    last_valid_start = n_bars - horizon
    if last_valid_start <= 0:
        raise ValueError("not enough bars for directional permutation horizon")

    valid_start = np.arange(last_valid_start, dtype=int)
    valid_mask = (
        np.isfinite(close_arr[:last_valid_start])
        & np.isfinite(close_arr[horizon:])
        & np.isfinite(atr_arr[:last_valid_start])
        & (atr_arr[:last_valid_start] > 0)
    )
    valid_start = valid_start[valid_mask]
    if len(valid_start) == 0:
        raise ValueError("no valid bars for directional permutation test")

    long_outcomes = (close_arr[valid_start + horizon] - close_arr[valid_start]) / (2.0 * atr_arr[valid_start])
    ...
    for sig in signals:
        ...
        r8 = _forward_return_r(data, atr_series, int(sig["bar_index"]), direction, horizon)
        if r8 is None or not np.isfinite(r8):
            continue
        observed_scores.append(float(r8))
        if direction == "LONG":
            n_long += 1
    ...
    for i in range(n_permutations):
        ...
        if n_long:
            sampled.append(rng.choice(long_outcomes, size=n_long, replace=False))
        if n_short:
            sampled.append(rng.choice(short_outcomes, size=n_short, replace=False))
